...
public class ChatController : ControllerBase
{
    ...
    public IActionResult Connect(string nome)
    {
        ...
        _logger.LogInformation($"Client {clientHandler.ClientId} connected.");
        return Ok(new { clientId = clientHandler.ClientId, message = "Client connected successfully." });
    }

    [HttpPost("start-conversation")]
    public IActionResult StartConversation(int clientId, int targetClientId)
    {
        var client = _clientManager.GetClient(clientId);
        if (client == null) return NotFound("Client not found.");
        
        client.CurrentConversationWith = targetClientId;
